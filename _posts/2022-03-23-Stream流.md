...
![](C:\Users\永涛\Desktop\技术积累\Stream流\图片\stream流常用方法.jpg)
...
​			2.Consumer是一个消费接口（可以获取流中的元素进行遍历操作，输出出去），可以使用Lambda			表达式
...
​	<R, A> R collect(Collector<? super T, A, R> collector);     把流中的数据手机到单列集合中，返回值类型是R。R指定为什么类型，就是手机到什么类型的集合
